...
- **화면 잘림 개선**: Zoom 갤러리 뷰 개별 칸에서 잘린 얼굴 감지 개선
...
2. `출결관리.exe` 더블클릭 실행
3. 별도 설치 불필요 (Python, 라이브러리 모두 번들링됨)
...
- GPU 지원 제거, CPU 전용 모드로 전환
  - NVIDIA GTX 960 관련 내용 모두 제거
  - onnxruntime-gpu → onnxruntime 변경
  - 성능 목표: 얼굴 감지 ~5초로 업데이트
  - CUDA Toolkit 요구사항 제거
  - 시스템 요구사항 간소화 (GPU/VRAM 제거)