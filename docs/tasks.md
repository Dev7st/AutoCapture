...
### 4.6 최종 검증 및 배포 준비
- [x] 실제 환경에서 최종 테스트 (듀얼 모니터, Zoom 화면)
- [x] UX 개선 (실제 사용 피드백 반영)
  - [x] 캡처 성공/실패 알림창 제거 → 상태 메시지로 변경
  - [x] "(권장)" 문구 삭제, 기본 모드 변경 (유연 → 정확)
  - [x] 이모지 컬러 표시 (Segoe UI Emoji 폰트 적용)
  - [x] InitDialog 기본 모드 버그 수정 (config.py 기본값 수정)
  - [x] 현재 상태 항목 제거 (교시별 상태와 중복)
  - [x] UI 레이아웃 미세 조정 (이모지 크기, 간격)
  - [x] 감지중 상태 표시 개선 (update_idletasks()로 UI 강제 업데이트)
  - [x] CSV 파일 잠금 처리 (Excel 열어도 프로그램 계속 실행)
  - [x] 메인 윈도우 크기 조정 (750x800, 1920x1080 저DPI 환경 대응)
  - [x] 재시도 버튼 이모지 중복 추가 방지
  - [x] 재시도 실패 시 상태 복원 로직 삭제 (최신 결과 표시)

### 4.7 얼굴 감지 필터링 개선
- [x] 특징점 가시성 체크 기능 구현
- [x] 가림 감지 필터링 기능 구현 (detection score)
- [x] bbox 기반 필터링으로 변경 (Zoom 칸 대응)
- [x] 눈/코는 bbox만, 입은 bbox + margin(5px) 필터링 적용
- [x] min_det_score 0.65로 설정 (가림 감지)
- [x] 실제 환경 테스트 및 파라미터 조정
