...
- [ ] 캡처 실패 알림 구현 (requirements.md 6.1.3)
  - [ ] 제목: "캡처 실패"
  - [ ] 감지 인원 / 기준 인원 표시
  - [ ] 모드별 안내 메시지 (유연 모드: 최소 N명 필요)
  - [ ] "10초 후 재시도합니다." 안내
  - [ ] alert_type="warning"
- [ ] `MainWindow._check_timeout_periods()` 알림창 추가
- [ ] 시간 초과 알림 구현 (requirements.md 6.1.4)
  - [ ] 제목: "캡처 시간 초과"
  - [ ] "N교시 캡처 시간이 종료되었으나 캡처하지 못했습니다." 메시지
  - [ ] 수동 처리 안내 (재시도 버튼 / 건너뛰기 사용)
  - [ ] alert_type="warning"
- [ ] 알림창 동작 테스트
  - [ ] 캡처 실패 시 알림창 표시 확인
  - [ ] 시간 초과 시 알림창 표시 확인
...
### 3.3 저장 폴더 열기 기능 연결
- [ ] [📂 저장 폴더 열기] 버튼 이벤트 핸들러
- [ ] `os.startfile()` 또는 `subprocess`로 탐색기 열기
- [ ] 폴더 없을 시 에러 알림
...
### 4.3 코드 품질 검토
- [ ] rules.md 체크리스트 전수 점검 (16장)
- [ ] 중복 코드 제거 및 리팩토링

### 4.4 통합 테스트
- [ ] 전체 교시 캡처 시나리오 테스트 (1교시~퇴실)
- [ ] 10시간 안정성 테스트 및 메모리 누수 확인
- [ ] 예외 상황 테스트
  - 권한 없는 폴더, 디스크 공간 부족, GPU 불가, 모니터 연결 해제, Zoom 없음

### 4.5 문서화
- [ ] README.md 작성 (프로젝트 소개, 시스템 요구사항, 설치 방법, 사용 방법, 트러블슈팅)
