...
4. 잘못된 모니터 선택
5. 기준 인원 설정 오류
6. 얼굴이 부분적으로 가려진 경우 (손, 컵 등)
7. 구도상 입이 화면 밖으로 잘린 경우
...
6. 얼굴 필터링 확인
   - 손으로 입을 가리면 필터링될 수 있음
   - 10초 후 자동 재시도 (손을 내리면 캡처 성공)
   - Zoom 갤러리 뷰에서 입이 칸 하단 밖으로 나가지 않도록 조정
   - 카메라 각도 조정 (얼굴 전체가 칸 안에 들어오도록)
```
...
**원인:**
- Excel이 CSV 파일을 독점 잠금
- 프로그램이 로그를 기록하려고 할 때 파일 접근 불가

**해결 방법:**
```
1. Excel에서 CSV 파일 닫기 (권장)
   - 프로그램 실행 중에는 CSV 파일을 닫아두세요
   - 확인이 필요하면 프로그램 종료 후 열기
...
3. Excel 읽기 전용으로 열기
   - Excel 실행 → 파일 → 열기 → CSV 선택
   - 오른쪽 하단 "열기" 버튼 클릭 대신
   - 드롭다운 → "읽기 전용으로 열기" 선택

4. 복사본 만들어서 보기
   - CSV 파일 복사 → 복사본을 Excel로 열기
   - 원본 파일은 프로그램이 계속 사용 가능
```

**참고:**
- ⚠️ CSV 파일이 Excel로 열려있어도 프로그램은 정상 작동합니다
- 스크린샷(PNG)은 정상적으로 저장되며, 그 시간의 CSV 로그만 누락됩니다
- CSV 로그는 문제 추적용 보조 자료이므로, 일부 누락되어도 출석 관리에는 지장 없습니다
- 로그 누락을 완전히 방지하려면 프로그램 실행 중에는 CSV를 닫아두세요
...
**문서 버전**: 1.3<br>
**최종 수정일**: 2026-01-02<br>
**주요 변경사항**: GPU 지원 제거, CPU 전용 모드로 전환<br>
